...
class yz_BunnyGrip : public Scene
{

public:
	yz_BunnyGrip(const char* name) :
		Scene(name),
		mRadius(0.1f),
...
public:
	...
	virtual void Initialize(py::array_t<float> scene_params, int thread_idx = 0)
	{
        ...
        Instance plasticBunny(bunny_path);
        ...
        float scale = ptr[0];
        float globalStiffness = ptr[1];
        float clusterPlasticThreshold = ptr[2];
        float clusterPlasticCreep = ptr[3];
        float draw_mesh = ptr[4];

        plasticBunny.mScale = Vec3(scale);
        plasticBunny.mClusterSpacing = 1.0f;
        plasticBunny.mClusterRadius = 0.0f;
        plasticBunny.mClusterStiffness = 0.0f;
        plasticBunny.mGlobalStiffness = globalStiffness;
        plasticBunny.mClusterPlasticThreshold = clusterPlasticThreshold;
        plasticBunny.mClusterPlasticCreep = clusterPlasticCreep;
        plasticBunny.mTranslation.x = -scale/20.0f;
        plasticBunny.mTranslation.y = 0.1f;
        plasticBunny.mTranslation.z = -scale/20.0f;
        AddInstance(plasticBunny);
