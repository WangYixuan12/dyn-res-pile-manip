...
class yz_SoftFall : public Scene
{
public:
    yz_SoftFall(const char* name) :
        Scene(name),
        mRadius(0.1f),
...
public:
    void Initialize(py::array_t<float> scene_params, int thread_idx = 0)
    {
        ...
        if (type == 0) {
            ...
            make_path(path, "/data/armadillo.ply");
        }
        else if (type == 2) {
            make_path(path, "/data/softs/octopus.obj");
        }
        else if (type == 3) {
            make_path(path, "/data/dragon.obj");
        }

        Instance instance(path);
        instance.mScale = Vec3(scale);
        instance.mClusterSpacing = 3.0f;
        instance.mClusterRadius = 0.0f;
        instance.mClusterStiffness = 0.2f;

        instance.mTranslation = Vec3(x, y, z);
