...
des_dir = 'test_FluidIceShake_multiview'
...
box_dis_z = sz_f * 0.055 + rand_float(0., 0.3)
...
for i in range(time_step):
    x_box_last = x_box
    x_box += v_box * dt
    v_box += rand_float(-0.1, 0.1) - x_box * 0.1
...
bg = scipy.misc.imread(os.path.join(des_dir, 'bg.tga'))
bg = bg[..., :3][..., ::-1]
...
for i in range(time_step - 1):
    ...
    # construct mask by comparing with the background
    img_noShadow = scipy.misc.imread(os.path.join(des_dir, 'step_noShadow_%d.tga' % i))
    img_noShadow = img_noShadow[..., :3][..., ::-1]
